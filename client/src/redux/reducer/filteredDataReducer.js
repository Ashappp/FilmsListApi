    ...
    case "GET_MOVIE_FROM_TITLE":
      if (action.param === '') {
        ...
        const arrTitle = [...action.payload].filter(el =>
          el.title.toLowerCase().indexOf(action.param.toLowerCase()) >= 0
        );
        return arrTitle;
      } else {
        return [...action.payload];
      ...
      case "GET_MOVIE_FROM_STAR":
        if (action.param === '') {
          ...
          const arrStar = [...action.payload].filter(el =>
            el.stars.find(item => item.toLowerCase().indexOf(action.param.toLowerCase()) >= 0)
          );
          return arrStar;
        } else {
          return [...action.payload];
        ...
        case "GET_SORT_MOVIE_ZA":
          const arrSortZA = [...action.payload].sort((a, b) => a.title < b.title ? 1 : -1);
          return arrSortZA;
